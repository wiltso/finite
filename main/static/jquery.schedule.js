...
;(function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    init: function () {
      ...
      // duration validation
      if ($.inArray(this.settings.periodDuration, [1, 5, 15, 30, 60]) === -1) {
        throw new Error('Invalid periodDuration');
      }
      this.startHour = 7;
      this.endHour = 20;
      this.periodInterval = 60;
      this.periodHeight = (this.endHour - this.startHour) * this.periodInterval;
    ...
    create: function () {
      ...
      for (var k = 0; k < this.settings.days; k++) {
        if(this.settings.seeProfile === false){
          if(this.settings.usernameList[k] === ''){
            $('<div class="jqs-grid-day"><a href="/users/profile">' + this.settings.daysList[k] + '</a></div>').
            appendTo($('.jqs-grid-head', this.element));
          }else{
            $('<div class="jqs-grid-day"><a href="/users/view/profile/' + this.settings.usernameList[k] + '/">' + this.settings.daysList[k] + '</a></div>').
              appendTo($('.jqs-grid-head', this.element));
          }
        }else{
          $('<div class="jqs-grid-day"><a href="'+ this.settings.usernameList[k] + '/">' + this.settings.daysList[k] + '</a></div>').
          appendTo($('.jqs-grid-head', this.element));
        }
      }
    },
    ...
    add: function (parent, position, height, options) {
      ...
      var periodTime = '<div class="jqs-period-time">' + this.startTime + "-"+this.endTime + '</div>';
    ...
    periodFormat: function (position) {
      ...
      var hour = Math.floor(position / this.periodInterval + this.startHour);
    ...
    positionFormat: function (time) {
      ...
      var position = 0;
      position += (hour - this.startHour) * this.periodInterval;
      position += mn;
